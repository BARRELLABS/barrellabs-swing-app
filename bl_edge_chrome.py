...
""",
    ...
    # NOTE: a JS "ghost-masthead killer" used to live here to remove stale
    # duplicate nav bars left by in-session nav. It is no longer needed: the
    # root cause (the masthead rendering at different element-tree positions on
    # different pages) was fixed by dispatching all nav pages from the same
    # place in app.py, so Streamlit reuses this masthead in place instead of
    # tearing it down. The lightweight CSS guards above (hide button-less /
    # duplicate mastheads) remain as a zero-cost backstop.
