...
def _mini_sparkline(scores: List[int]) -> str:
    """Responsive viewBox sparkline of edge scores (no fixed px, scales to card)."""
    vals = [float(s) for s in scores if s]
    if len(vals) < 2:
        return ""
    ...
    lx, ly = pts[-1]
    return (
        '<svg class="ld-spark" viewBox="0 0 100 30" '
        'xmlns="http://www.w3.org/2000/svg">'
        f'<polyline points="{poly}" fill="none" stroke="var(--gold)" '
        'stroke-width="1.3" stroke-linecap="round" stroke-linejoin="round"/>'
        f'<circle cx="{lx:.1f}" cy="{ly:.1f}" r="1.8" fill="var(--red)"/>'
        '</svg>'
    )
...
_LEAN_DASH_CSS = """
<style>
.ld-wrap { max-width: 900px; margin: 0 auto; padding: 0.3rem 0 2.5rem; font-family: var(--sans); position: relative; z-index: 1; }
.ld-hello { font-family: var(--serif); font-size: 1.95rem; color: var(--bone); letter-spacing: -0.01em; margin: 0.3rem 0 1.1rem; }
.ld-eyebrow { font-family: var(--mono); font-size: 0.6rem; letter-spacing: 0.22em; text-transform: uppercase; color: var(--red); font-weight: 600; margin-bottom: 0.5rem; }
.ld-muted { color: var(--bl-ink-60); font-weight: 400; font-size: 0.85rem; }
.ld-card { border: 1px solid var(--bl-line); border-radius: var(--bl-radius-md); background: linear-gradient(180deg, rgba(244,239,230,0.022), rgba(244,239,230,0.006)); padding: 1.2rem 1.3rem; margin-bottom: 0.9rem; }
.ld-hero { display: grid; grid-template-columns: 1fr 1fr; gap: 0.9rem; margin-bottom: 0.9rem; }
.ld-score-block, .ld-match { border: 1px solid var(--bl-line); border-radius: var(--bl-radius-md); background: linear-gradient(180deg, rgba(244,239,230,0.022), rgba(244,239,230,0.006)); padding: 1.3rem 1.4rem; }
.ld-score { font-family: var(--serif); font-size: 3.4rem; line-height: 1; color: var(--bone); letter-spacing: -0.02em; }
.ld-score-max { font-size: 1.1rem; color: var(--bl-ink-40); margin-left: 0.15rem; }
.ld-tier { margin-top: 0.5rem; color: var(--gold); font-weight: 600; font-size: 0.95rem; }
.ld-match-name { font-family: var(--serif); font-style: italic; font-size: 1.7rem; color: var(--bone); line-height: 1.05; }
.ld-match-pct { margin-top: 0.45rem; font-size: 1.55rem; font-weight: 700; color: var(--gold); }
.ld-match-pct span { font-size: 0.78rem; color: var(--bl-ink-60); font-weight: 500; }
.ld-match-bio { margin-top: 0.5rem; color: var(--bl-ink-60); font-size: 0.82rem; line-height: 1.45; }
.ld-stats { display: grid; grid-template-columns: repeat(4, 1fr); gap: 0.6rem; margin-bottom: 0.9rem; }
.ld-stat { border: 1px solid var(--bl-line); border-radius: var(--bl-radius-sm); background: rgba(244,239,230,0.02); padding: 0.9rem 0.5rem; text-align: center; color: var(--bl-ink-60); font-size: 0.7rem; line-height: 1.3; }
.ld-stat-num { display: block; font-size: 1.7rem; font-weight: 700; color: var(--bone); letter-spacing: -0.02em; margin-bottom: 0.15rem; }
.ld-spark { width: 100%; height: auto; display: block; margin-top: 0.4rem; }
.ld-focus-area { font-family: var(--serif); font-style: italic; font-size: 1.5rem; color: var(--bone); margin-bottom: 0.3rem; text-transform: capitalize; }
.ld-sess-row { display: flex; align-items: center; gap: 0.8rem; padding: 0.6rem 0; border-top: 1px solid var(--bl-line); }
.ld-sess-row.first { border-top: none; }
.ld-sess-score { font-weight: 700; font-size: 1.2rem; color: var(--gold); min-width: 2.4rem; }
.ld-sess-meta { color: var(--bone-dim); font-size: 0.88rem; }
@media (max-width: 640px) {
  .ld-hero { grid-template-columns: 1fr; }
  .ld-stats { grid-template-columns: repeat(2, 1fr); }
  .ld-hello { font-size: 1.6rem; }
  .ld-score { font-size: 3rem; }
}
</style>
"""


def _render_lean_dashboard(user: Dict[str, Any],
                           history: List[Dict[str, Any]],
                           latest: Dict[str, Any]) -> None:
    """Lean, real-data dashboard. Every value here is computed from the
    ...
    edge = _compose_edge_score(latest)
    ...
    try:
        ...
        axes = {}
    focus_area, focus_val = ("", 0)
    if axes:
        focus_area, focus_val = min(axes.items(), key=lambda kv: kv[1])

    tier_line = f"{pts} pts to {next_tier}" if pts and pts > 0 else "Top tier — keep it up"

    # ---- build the section HTML (skip blocks with no real data) ----
    score_block = (
        '<div class="ld-score-block"><div class="ld-eyebrow">Edge Score</div>'
        f'<div class="ld-score">{edge}<span class="ld-score-max">/100</span></div>'
        f'<div class="ld-tier">{tier} · <span class="ld-muted">{tier_line}</span></div></div>'
    )
    match_html = ""
    if ref_name:
        bio = f'<div class="ld-match-bio">{ref_bio}</div>' if ref_bio else ""
        match_html = (
            '<div class="ld-match"><div class="ld-eyebrow">Closest MLB match</div>'
            f'<div class="ld-match-name">{ref_name}</div>'
            f'<div class="ld-match-pct">{match_pct}<span>% match</span></div>{bio}</div>'
        )
    # Two-up hero when there's a match; full-width score block when there isn't.
    if match_html:
        hero_html = f'<div class="ld-hero">{score_block}{match_html}</div>'
    else:
        hero_html = f'<div style="margin-bottom:0.9rem;">{score_block}</div>'

    if len(scores) >= 2:
        trend_inner = _mini_sparkline(scores)
        trend_html = (
            '<div class="ld-card"><div class="ld-eyebrow">Edge Score trend · last '
            f'{len(scores)} swings</div>{trend_inner}</div>'
        )
    else:
        trend_html = (
            '<div class="ld-card"><div class="ld-eyebrow">Edge Score trend</div>'
            '<div class="ld-muted">Your trend line builds with your next swing.</div></div>'
        )
    ...
    last_line = f' <span class="ld-muted">· last swing {last_date}</span>' if last_date else ""

    st.html(
        _LEAN_DASH_CSS
        + '<div class="ld-wrap">'
        + f'<div class="ld-hello">Hi {first}.{last_line}</div>'
        + hero_html
        + '<div class="ld-stats">'
        + f'<div class="ld-stat"><span class="ld-stat-num">{swings_wk}</span>Swings this week</div>'
        + f'<div class="ld-stat"><span class="ld-stat-num">{prs}</span>Personal records</div>'
        + f'<div class="ld-stat"><span class="ld-stat-num">{streak}</span>Day streak</div>'
        + f'<div class="ld-stat"><span class="ld-stat-num">{total}</span>Total swings</div>'
        + '</div>'
        + trend_html
        + focus_html
        + recent_html
    ...
    # ---- real interactions (Streamlit widgets, must live outside the HTML) ----
    if st.button("＋ Analyze next swing", key="ld_analyze", type="primary",
    ...
    with c1:
        if st.button("Open latest report", key="ld_open_latest",
                     use_container_width=True):
            st.session_state["view_swing_record"] = latest
            st.session_state["view_swing_report_id"] = (
                latest.get("id") or latest.get("report_id") or "")
            rp = latest.get("_record_path")
            if rp:
            ...
            st.rerun()
    with c2:
        if st.button("See your drills", key="ld_drills",
                     use_container_width=True):
            st.session_state["page"] = "development_tracker"
            st.rerun()
    with c3:
        if st.button("All sessions", key="ld_all_sessions",
                     use_container_width=True):
...
def render_dashboard_v3(user: Dict[str, Any],
    ...
    # Lean, real-data dashboard for the normal view. If it ever raises on a
    # real record shape, fall through to the legacy mock-template render below
    # so the user never sees a blank/errored dashboard.
    if force_record is None:
        try:
            _render_lean_dashboard(user, history, latest)
            return
        except Exception:
            import traceback
            traceback.print_exc()
