...
def _build_swing_video(vid_url: Optional[str]) -> str:
    """The actual swing clip — the credibility piece a coach wants. Renders only
    when a video URL is available (Pro swings with a stored, signed video)."""
    if not vid_url:
        return ""
    # #t=0.5 makes the browser paint the frame at 0.5s as the still, instead of
    # opening on a black screen. The signed URL's ?token stays intact (# is a
    # client-side fragment).
    src = html.escape(vid_url + "#t=0.5", quote=True)
    return f"""
<div class="srd-section">
  <div>
    <div class="srd-eyebrow">§ 02 · The swing</div>
    <h2 class="srd-section-title">Watch the rep</h2>
  </div>
  <div class="srd-section-sub">Scrub to contact to see what the numbers describe</div>
</div>
<div class="srd2-video-card">
  <video class="srd2-video" controls preload="metadata" playsinline
         src="{src}"></video>
</div>
"""
